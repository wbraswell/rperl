...
for my $i ( 0 .. 2 ) {
    ...
    if ( $i == 0 ) {
        ...
        lives_and( sub { is( types_number(), 'PERL', q{types_number() returns 'PERL'} ) }, q{types_number() lives} );
        lives_and( sub { is( ops_number(),   'PERL', q{ops_number() returns 'PERL'} ) }, q{ops_number() lives} );
        lives_and( sub { is( types_string(), 'PERL', q{types_string() returns 'PERL'} ) }, q{types_string() lives} );
        lives_and( sub { is( ops_string(),   'PERL', q{ops_string() returns 'PERL'} ) }, q{ops_string() lives} );
        lives_and( sub { is( types_array(),  'PERL', q{types_array() returns 'PERL'} ) }, q{types_array() lives} );
        lives_and( sub { is( ops_array(),    'PERL', q{ops_array() returns 'PERL'} ) }, q{ops_array() lives} );
    }
    elsif ( $i == 1 ) {
        diag(
...
#        lives_and( sub { is( types::types_enable('PERL'), undef, q{types::types_enable('PERL') returns undef} ) }, q{types::types_enable('PERL') lives} );
        lives_ok( sub { types::types_enable('PERL') }, q{types::types_enable('PERL') lives} );  # NEED FIX?  should it be just this lives_ok() or the lives_and() above?
        ...
        lives_and( sub { is( types_number(), 'PERL', q{types_number() returns 'PERL'} ) }, q{types_number() lives} );
        lives_and( sub { is( ops_number(),   'CPP', q{ops_number() returns 'CPP'} ) },  q{ops_number() lives} );
        lives_and( sub { is( types_string(), 'PERL', q{types_string() returns 'PERL'} ) }, q{types_string() lives} );
        lives_and( sub { is( ops_string(),   'CPP', q{ops_string() returns 'CPP'} ) },  q{ops_string() lives} );
        lives_and( sub { is( types_array(),  'PERL', q{types_array() returns 'PERL'} ) }, q{types_array() lives} );
        lives_and( sub { is( ops_array(),    'CPP', q{ops_array() returns 'CPP'} ) },  q{ops_array() lives} );
    }
    else {
        diag( "\n[[[ Beginning Pure-C++ Array Type Tests, RPerl Type System Using C++ Data Types & C++ Operations ]]]\n " );
        lives_ok( sub { types::types_enable('CPP') }, q{types::types_enable('CPP') lives} );
        ...
        lives_ok( sub { RPerl::DataStructure::Array_cpp::cpp_link(); }, q{RPerl::DataStructure::Array_cpp::cpp_link() lives} );
        lives_and( sub { is( types_number(), 'CPP', q{types_number() returns 'CPP'} ) }, q{types_number() lives} );
        lives_and( sub { is( ops_number(),   'CPP', q{ops_number() returns 'CPP'} ) }, q{ops_number() lives} );
        lives_and( sub { is( types_string(), 'CPP', q{types_string() returns 'CPP'} ) }, q{types_string() lives} );
        lives_and( sub { is( ops_string(),   'CPP', q{ops_string() returns 'CPP'} ) }, q{ops_string() lives} );
        lives_and( sub { is( types_array(),  'CPP', q{types_array() returns 'CPP'} ) }, q{types_array() lives} );
        lives_and( sub { is( ops_array(),    'CPP', q{ops_array() returns 'CPP'} ) }, q{ops_array() lives} );
    }
    ...
    lives_and( sub { is( stringify_int__array_ref( [2] ), '[2]', q{stringify_int__array_ref([2]) returns '[2]' } ) }, q{stringify_int__array_ref([2]) lives} );
    lives_and(
        sub {
            is( stringify_int__array_ref( [ 2, 2112, 42, 23, 877, 33, 1701 ] ),
                '[2, 2112, 42, 23, 877, 33, 1701]'
            );
        },
        q(stringify_int__array_ref([2, 2112, 42, 23, 877, 33, 1701]);)
    );
    throws_ok(
        sub {
        ...
        q(stringify_int__array_ref([2, 2112, 42.3, 23, 877, 33, 1701]);  # throw exception)
    );
    throws_ok(
        sub {
        ...
        '/input_av_element at index 2 was not an int/',
        q(stringify_int__array_ref([2, 2112, '42', 23, 877, 33, 1701]);  # throw exception)
    );
    lives_and(
        sub {
        ...
        q(typetest___int__array_ref__in___string__out([2, 2112, 42, 23, 877, 33, 1701]);)
    );
    throws_ok(
        sub {
    ...
    lives_and(
        ...
        q(typetest___int__array_ref__in___string__out([444, 33, 1701]);  # call again to test Perl stack is still functioning properly)
    );
    lives_and(
        sub {
    ...
    throws_ok(
        sub {
            stringify_number__array_ref( [ 2, 2112, '42', 23, 877, 33, 1701 ] );
        },
