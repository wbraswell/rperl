...
our void_method $cpp_load = sub {
    my $need_load_cpp = 0;
    if (    ( exists $main::{'RPerl__Algorithm__Sort__MODE_ID'} )
        and ( defined &{ $main::{'RPerl__Algorithm__Sort__MODE_ID'} } ) )
    {
#        RPerl::diag "in Sort_cpp::cpp_load, RPerl__Algorithm__Sort__MODE_ID() exists & defined\n";
#        RPerl::diag q{in Sort_cpp::cpp_load, have RPerl__Algorithm__Sort__MODE_ID() retval = '} . main::RPerl__Algorithm__Sort__MODE_ID() . "'\n";
        if ( $RPerl::MODES->{main::RPerl__Algorithm__Sort__MODE_ID()}->{ops} ne 'CPP' ) {
            $need_load_cpp = 1;
        }
    }
    else {
#        RPerl::diag "in Sort_cpp::cpp_load, RPerl__Algorithm__Sort__MODE_ID() does not exist or undefined\n";
        $need_load_cpp = 1;
    }
