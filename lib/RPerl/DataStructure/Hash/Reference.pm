...
#    RPerl::diag( 'in Hash::Reference->ast_to_rperl__generate(), received $self = ' . "\n" . RPerl::Parser::rperl_ast__dump($self) . "\n" );
    ...
    # unwrap HashReference_209 & HashReference_210 from SubExpression_140
    if ( $self_class eq 'SubExpression_140' ) {
        $self = $self->{children}->[0];
        $self_class = ref $self;
    }

    if ( $self_class eq 'HashReference_209' ) { # HashReference -> LBRACE HashEntry STAR-50 '}'
    ...
    elsif ( $self_class eq 'HashReference_210' ) { # HashReference -> LBRACE '}'
        my string $left_brace  = $self->{children}->[0];
        ...
        die RPerl::Parser::rperl_rule__replace(
            'ERROR ECVGEASRP00, CODE GENERATOR, ABSTRACT SYNTAX TO RPERL: grammar rule '
                . ($self_class)
                . ' found where HashReference_209, HashReference_210, or SubExpression_140 expected, dying'
        ) . "\n";
    }
