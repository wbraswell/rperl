...
#    RPerl::diag( 'in Hash::Entry->ast_to_rperl__generate(), received $self = ' . "\n" . RPerl::Parser::rperl_ast__dump($self) . "\n" );
    ...
    if ( $self_class ne 'HashEntry_203' ) {  # HashEntry -> VariableOrLiteralOrWord OP20_HASH_FATARROW OPTIONAL-47 SubExpression
        die RPerl::Parser::rperl_rule__replace(
            'ERROR ECVGEASRP00, CODE GENERATOR, ABSTRACT SYNTAX TO RPERL: grammar rule '
                . $self_class
                . ' found where HashEntry_203 expected, dying'
        ) . "\n";
    }
    ...
    if (   ( $key_class eq 'VariableOrLiteralOrWord_227' )
        or ( $key_class eq 'VariableOrLiteralOrWord_228' ) )
    {    # Variable or Literal
        $rperl_source_subgroup = $key->ast_to_rperl__generate($modes);
        RPerl::Generator::source_group_append( $rperl_source_group,
            $rperl_source_subgroup );
    }
    elsif ( $key_class eq 'VariableOrLiteralOrWord_229' ) {    # WORD
        ...
        die RPerl::Parser::rperl_rule__replace(
            q{ERROR ECVGEASRP00, CODE GENERATOR, ABSTRACT SYNTAX TO RPERL: grammar rule '}
                . ($key_class)
                . q{' found where VariableOrLiteralOrWord_227, VariableOrLiteralOrWord_228, or VariableOrLiteralOrWord_229 expected, dying}
        ) . "\n";
    }
