...
;
#	if (defined($RPerl::DataType::String_cpp::CPP_LINKED)) { RPerl::diag "in String_cpp::cpp_link(), have \$RPerl::DataType::String_cpp::CPP_LINKED = '" . $RPerl::DataType::String_cpp::CPP_LINKED . "'\n"; }
#		else { RPerl::diag "in String_cpp::cpp_link(), have \$RPerl::DataType::String_cpp::CPP_LINKED = 'UNDEF'\n"; }
...
1;
EOF
#		RPerl::diag "in String_cpp::cpp_link(), CPP not yet linked, about to call eval() on \$eval_string =\n<<< BEGIN EVAL STRING>>>\n" . $eval_string . "<<< END EVAL STRING >>>\n";

		eval($eval_string);  ## no critic
		die($@) if ($@);
	}
#	else { RPerl::diag "in String_cpp::cpp_link(), CPP already linked, DOING NOTHING\n"; }
};
