...
our string_hashref::method $ast_to_rperl__generate = sub {
    ...
    if ( $self_class eq 'VariableRetrieval_174' ) {    # VariableRetrieval -> OP02_ARRAY_THINARROW SubExpression ']'
    ...
    elsif ( $self_class eq 'VariableRetrieval_175' ) {    # VariableRetrieval -> OP02_HASH_THINARROW SubExpression '}'
    ...
    elsif ( $self_class eq 'VariableRetrieval_176' ) {    # VariableRetrieval -> OP02_HASH_THINARROW WORD '}'
        my string $arrow_left_brace = $self->{children}->[0];
        ...
        die RPerl::Parser::rperl_rule__replace( 'ERROR ECVGEASRP00, CODE GENERATOR, ABSTRACT SYNTAX TO RPERL: grammar rule '
                . ($self_class)
                . ' found where VariableRetrieval_174, VariableRetrieval_174, or VariableRetrieval_174 expected, dying' )
            . "\n";
    }
