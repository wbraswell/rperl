...
sub ast_to_rperl__generate {
    ...
    if ( $self_class eq 'VariableRetrieval_198' ) {    # VariableRetrieval -> OP02_ARRAY_THINARROW SubExpression ']'
    ...
    elsif ( $self_class eq 'VariableRetrieval_199' ) {    # VariableRetrieval -> OP02_HASH_THINARROW SubExpression '}'
    ...
    elsif ( $self_class eq 'VariableRetrieval_200' ) {    # VariableRetrieval -> OP02_HASH_THINARROW WORD '}'
        my string $arrow_left_brace = $self->{children}->[0];
        ...
        die RPerl::Parser::rperl_rule__replace( 'ERROR ECOGEASRP000, CODE GENERATOR, ABSTRACT SYNTAX TO RPERL: Grammar rule '
                . ($self_class)
                . ' found where VariableRetrieval_198, VariableRetrieval_198, or VariableRetrieval_198 expected, dying' )
            . "\n";
    }
