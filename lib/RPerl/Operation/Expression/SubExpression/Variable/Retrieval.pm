...
our string_hashref::method $ast_to_rperl__generate = sub {
    ...
    if ( $self_class eq 'VariableRetrieval_176' ) {    # VariableRetrieval -> OP02_ARRAY_THINARROW SubExpression ']'
    ...
    elsif ( $self_class eq 'VariableRetrieval_177' ) {    # VariableRetrieval -> OP02_HASH_THINARROW SubExpression '}'
    ...
    elsif ( $self_class eq 'VariableRetrieval_178' ) {    # VariableRetrieval -> OP02_HASH_THINARROW WORD '}'
        my string $arrow_left_brace = $self->{children}->[0];
        ...
        die RPerl::Parser::rperl_rule__replace( 'ERROR ECVGEASRP00, CODE GENERATOR, ABSTRACT SYNTAX TO RPERL: grammar rule '
                . ($self_class)
                . ' found where VariableRetrieval_176, VariableRetrieval_176, or VariableRetrieval_176 expected, dying' )
            . "\n";
    }
