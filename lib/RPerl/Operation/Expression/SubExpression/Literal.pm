...
#    RPerl::diag( 'in Literal->ast_to_rperl__generate(), received $self = ' . "\n" . RPerl::Parser::rperl_ast__dump($self) . "\n" );
    ...
    if (( $self_class eq 'SubExpression_157' )    # SubExpression -> Literal
        or ( $self_class eq 'VariableOrLiteral_249' ) # VariableOrLiteral -> Literal
        or ( $self_class eq 'VarOrLitOrOpStrOrWord_251' ) # VarOrLitOrOpStrOrWord -> Literal
        )
    {
        ...
        die RPerl::Parser::rperl_rule__replace(
            'ERROR ECOGEASRP000, CODE GENERATOR, ABSTRACT SYNTAX TO RPERL: Grammar rule '
                . $self_class
                . ' found where SubExpression_157, VariableOrLiteral_249, or VarOrLitOrOpStrOrWord_251 expected, dying'
        ) . "\n"; 
    }
