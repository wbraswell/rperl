...
our string_hashref::method $ast_to_rperl__generate = sub {
    ...
    # unwrap Variable_179 from SubExpression_140, VariableOrLiteral_229, or VarOrLitOrOpStrOrWord_231
    if ( ( $self_class eq 'SubExpression_140' ) or ( $self_class eq 'VariableOrLiteral_229' ) or ( $self_class eq 'VarOrLitOrOpStrOrWord_231' ) ) {
        $self = $self->{children}->[0];
    }

    $self_class = ref $self;
    if ( $self_class eq 'Variable_179' ) {
        ...
        die RPerl::Parser::rperl_rule__replace( 'ERROR ECOGEASRP00, CODE GENERATOR, ABSTRACT SYNTAX TO RPERL: Grammar rule '
                . ($self_class)
                . ' found where SubExpression_140, VariableOrLiteral_229, VarOrLitOrOpStrOrWord_231, or Variable_179 expected, dying' )
            . "\n";
    }
...
#            RPerl::diag( 'in Variable->ast_to_cpp__generate__CPPOPS_CPPTYPES(), for loop $i = ' . $i . ', have $variable_retrieval = ' . "\n" . RPerl::Parser::rperl_ast__dump($variable_retrieval) . "\n" );
            ...
            if ( $i < $i_max ) {
                if ( $types->[$i] =~ /_arrayref$/ ) { # array
                    ...
                    if ( ( ref $variable_retrieval ) eq 'VariableRetrieval_181' ) {    # VariableRetrieval -> OP02_HASH_THINARROW SubExpression '}'
                        my object $subexpression = $variable_retrieval->{children}->[1];
                        if (( ( ref $subexpression ) eq 'SubExpression_139' )          # SubExpression -> Literal
                            or ( ( ref $subexpression ) eq 'VariableOrLiteral_230' )          # VariableOrLiteral -> Literal
                            or ( ( ref $subexpression ) eq 'VarOrLitOrOpStrOrWord_232' )    # VarOrLitOrOpStrOrWord -> Literal
                            )
                        {
...
#                            RPerl::diag( 'in Variable->ast_to_cpp__generate__CPPOPS_CPPTYPES() #0, for loop $i = ' . $i . ', have $modes->{_symbol_table}->{ $types->[$i] }->{_properties} = ' . $modes->{_symbol_table}->{ $types->[$i] }->{_properties} . "\n" );
                    ...
                    elsif ( ( ref $variable_retrieval ) eq 'VariableRetrieval_182' ) {        # VariableRetrieval -> OP02_HASH_THINARROW WORD '}'
                        if ((substr $types->[$i], -2, 2) ne '::') { $types->[$i] .= '::'; }  # _properties in _symbol_table are keyed by class/package names ending in ::
