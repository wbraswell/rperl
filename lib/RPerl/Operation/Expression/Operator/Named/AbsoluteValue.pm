...
# or ERROR ECOPARP00 'Unexpected Token:  ,'; short-circuited second by ERROR ECOGEASRP13 on Operator_101 & OperatorVoid_140 multiple-argument productions below;
...
#    RPerl::diag( 'in Operator::Named::AbsoluteValue->ast_to_rperl__generate(), received $operator_named = ' . "\n" . RPerl::Parser::rperl_ast__dump($operator_named) . "\n" );
    ...
    if ( $operator_named_class eq 'Operation_96' ) {    # Operation -> OP01_NAMED_SCOLON
        ...
        die RPerl::Parser::rperl_rule__replace( 'ERROR ECOGEASRP13a, CODE GENERATOR, ABSTRACT SYNTAX TO RPERL: named operator '
...
#    RPerl::diag( 'in Operator::Named::AbsoluteValue->ast_to_cpp__generate__CPPOPS_CPPTYPES(), received $operator_named = ' . "\n" . RPerl::Parser::rperl_ast__dump($operator_named) . "\n" );
    ...
    if ( $operator_named_class eq 'Operation_96' ) {    # Operation -> OP01_NAMED_SCOLON
        ...
        $cpp_source_group->{CPP} .= '(';  # DEV NOTE: must wrap operand in extra parens to avoid error "cannot resolve overloaded function ‘abs’ based on conversion to type ..."
        ...
        die RPerl::Parser::rperl_rule__replace( 'ERROR ECOGEASCP14a, CODE GENERATOR, ABSTRACT SYNTAX TO C++: named operator '
                . $operator_named->{children}->[1]
                . ' does not accept multiple arguments, dying' )
            . "\n";
    }
