...
#    RPerl::diag( 'in Loop->ast_to_rperl__generate(), received $self = ' . "\n" . RPerl::Parser::rperl_ast__dump($self) . "\n" );
    ...
    # unwrap Loop_162, Loop_163, and Loop_164 from Statement_151
    if ( $self_class eq 'Statement_151' ) {
        $self = $self->{children}->[0];
        $self_class = ref $self;
    }

    if (   ( $self_class eq 'Loop_162' )  # Loop -> LoopFor
        or ( $self_class eq 'Loop_163' )  # Loop -> LoopForEach
        or ( $self_class eq 'Loop_164' ) )  # Loop -> LoopWhile
    {
        ...
        die RPerl::Parser::rperl_rule__replace(
            'ERROR ECVGEASRP00, CODE GENERATOR, ABSTRACT SYNTAX TO RPERL: grammar rule '
                . $self_class
                . ' found where Loop_162, Loop_163, or Loop_164 expected, dying'
        ) . "\n";
    }
