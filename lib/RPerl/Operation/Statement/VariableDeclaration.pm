...
our string_hashref::method $ast_to_rperl__generate = sub {
    ...
    # unwrap VariableDeclaration_177, VariableDeclaration_178, VariableDeclaration_179, and VariableDeclaration_180 from Statement_151
    if ( $self_class eq 'Statement_151' ) {    # Statement -> VariableDeclaration
        $self       = $self->{children}->[0];
        $self_class = ref $self;
    }

    if ( $self_class eq 'VariableDeclaration_177' ) {    # VariableDeclaration -> MY Type VARIABLE_SYMBOL ';'
    ...
    elsif ( $self_class eq 'VariableDeclaration_178' ) {    # VariableDeclaration -> MY Type VARIABLE_SYMBOL OP19_VARIABLE_ASSIGN SubExpressionOrStdin ';'
        my string $my                     = $self->{children}->[0];
        my string $type                   = $self->{children}->[1]->{children}->[0];
        my string $symbol                 = $self->{children}->[2];
    ...
    elsif ( $self_class eq 'VariableDeclaration_179' ) { # VariableDeclaration -> MY Type VARIABLE_SYMBOL OP02_ARRAY_THINARROW SubExpression ']' OP19_VARIABLE_ASSIGN 'undef' ';'
        my string $my                 = $self->{children}->[0];
        my string $type               = $self->{children}->[1]->{children}->[0];
        my string $symbol             = $self->{children}->[2];
    ...
    elsif ( $self_class eq 'VariableDeclaration_180' ) {    # VariableDeclaration -> MY TYPE_FHREF FHREF_SYMBOL ';'
        my string $my           = $self->{children}->[0];
        ...
        die RPerl::Parser::rperl_rule__replace( 'ERROR ECVGEASRP00, CODE GENERATOR, ABSTRACT SYNTAX TO RPERL: grammar rule '
                . $self_class
                . ' found where VariableDeclaration_177, VariableDeclaration_178, VariableDeclaration_179, or VariableDeclaration_180 expected, dying' )
            . "\n";
    }
...
#        RPerl::diag( 'in VariableDeclaration->ast_to_cpp__generate__CPPOPS_CPPTYPES(), have $subexpression_or_stdin = ' . "\n" . RPerl::Parser::rperl_ast__dump($subexpression_or_stdin) . "\n" );
        ...
        if ( ((ref $subexpression_or_stdin) eq 'SubExpressionOrStdin_141' ) and
            ( exists $subexpression_or_stdin->{children} ) and
            ( exists $subexpression_or_stdin->{children}->[0]->{children} ) and
            ( $subexpression_or_stdin->{children}->[0]->{children}->[0]->isa('RPerl::Operation::Expression::SubroutineCall::MethodCall::ConstructorCall') ) )
        {
