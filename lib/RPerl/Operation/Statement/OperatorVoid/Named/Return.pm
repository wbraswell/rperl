...
# START HERE: decide how to handle return parenthesis, as discussed in comments below
# START HERE: decide how to handle return parenthesis, as discussed in comments below
# START HERE: decide how to handle return parenthesis, as discussed in comments below
...
our string_hashref_method $ast_to_rperl__generate = sub {
    ...
    my string $name = $operator_void_named->{children}->[0];
    my string $semicolon;

    $rperl_source_group->{PMC} .= $name . q{ };
    ...
    if (exists $operator_void_named->{children}->[2]) {
        ...
        $semicolon = $operator_void_named->{children}->[1];
    }
