...
#    RPerl::diag( 'in Loop::While->ast_to_rperl__generate(), received $self = ' . "\n" . RPerl::Parser::rperl_ast__dump($self) . "\n" );
    ...
    # unwrap LoopWhile_166 and LoopWhile_167 from Loop_162
    if ( $self_class eq 'Loop_162' ) {    # Loop -> LoopWhile
        $self = $self->{children}->[0];
        $self_class = ref $self;
    }

    if ( $self_class eq 'LoopWhile_166' ) {  # LoopWhile -> 'while' LPAREN SubExpression ')' CodeBlock
    ...
    elsif ( $self_class eq 'LoopWhile_167' ) {  # LoopWhile -> 'while' LPAREN_MY Type VARIABLE_SYMBOL OP19_VARIABLE_ASSIGN SubExpressionOrStdin ')' CodeBlock
        my string $while         = $self->{children}->[0];
        ...
        die RPerl::Parser::rperl_rule__replace(
            'ERROR ECVGEASRP00, CODE GENERATOR, ABSTRACT SYNTAX TO RPERL: grammar rule '
                . $self_class
                . ' found where LoopWhile_166 or LoopWhile_167 expected, dying' )
            . "\n";
    }
...
#    RPerl::diag( 'in Loop::While->ast_to_cpp__generate__CPPOPS_CPPTYPES(), received $loop_label = ' . $loop_label . "\n" );
    ...
    # unwrap LoopWhile_166 from Loop_162
    if ( $self_class eq 'Loop_162' ) {    # Loop -> LoopWhile
        $self = $self->{children}->[0];
        $self_class = ref $self;
    }
