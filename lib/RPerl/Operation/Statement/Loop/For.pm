...
our string_hashref::method $ast_to_rperl__generate = sub {
    ...
    # unwrap LoopFor_169 and LoopFor_170 from Loop_166
    if ( $self_class eq 'Loop_166' ) {    # Loop -> LoopFor
        $self       = $self->{children}->[0];
        $self_class = ref $self;
    }

    if ( $self_class eq 'LoopFor_169' ) {
    ...
    elsif ( $self_class eq 'LoopFor_170' ) {
...
#        RPerl::diag( 'in Loop::For->ast_to_rperl__generate(), have $opnamed_or_subexp_scolon0_type = ' . "\n" . RPerl::Parser::rperl_ast__dump($opnamed_or_subexp_scolon0_type) . "\n" );
        ...
        if (   ( $opnamed_or_subexp_scolon0_type eq 'OpNamedScolonOrSubExp_238' )
            or ( $opnamed_or_subexp_scolon0_type eq 'OpNamedScolonOrSubExp_239' ) )
        {
        ...
        elsif ( $opnamed_or_subexp_scolon0_type eq 'OpNamedScolonOrSubExp_240' ) {    # OpNamedScolonOrSubExp -> SubExpression ';'
            ...
            die RPerl::Parser::rperl_rule__replace( 'ERROR ECOGEASRP00, CODE GENERATOR, ABSTRACT SYNTAX TO RPERL: Grammar rule '
                    ...
                    . ' found where OpNamedScolonOrSubExp_238, OpNamedScolonOrSubExp_239, or OpNamedScolonOrSubExp_240 expected, dying' )
                . "\n";
        }
...
#        RPerl::diag( 'in Loop::For->ast_to_rperl__generate(), have $opnamed_or_subexp_scolon1_type = ' . "\n" . RPerl::Parser::rperl_ast__dump($opnamed_or_subexp_scolon1_type) . "\n" );
        ...
        if (   ( $opnamed_or_subexp_scolon1_type eq 'OpNamedScolonOrSubExp_238' )
            or ( $opnamed_or_subexp_scolon1_type eq 'OpNamedScolonOrSubExp_239' ) )
        {
            # OpNamedScolonOrSubExp -> OP01_NAMED_SCOLON
            # OpNamedScolonOrSubExp -> OP10_NAMED_UNARY_SCOLON
            $rperl_source_group->{PMC} .= $opnamed_or_subexp_scolon1->{children}->[0];
        }
        elsif ( $opnamed_or_subexp_scolon1_type eq 'OpNamedScolonOrSubExp_240' ) {    # OpNamedScolonOrSubExp -> SubExpression ';'
        ...
        die RPerl::Parser::rperl_rule__replace( 'ERROR ECOGEASRP00, CODE GENERATOR, ABSTRACT SYNTAX TO RPERL: Grammar rule '
                . $self_class
                . ' found where Loop_166, LoopFor_169, or LoopFor_170 expected, dying' )
            . "\n";
    }
