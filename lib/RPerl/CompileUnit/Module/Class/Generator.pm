...
#    RPerl::diag( 'in Class::Generator->ast_to_rperl__generate(), have $parent_include = ' . "\n" . RPerl::Parser::rperl_ast__dump($parent_include) . "\n" );
    if (ref $parent_include eq 'Include_39') {
        my string $parent_include_use_keyword = $parent_include->{children}->[0];
        my string $parent_include_parent_name = $parent_include->{children}->[1]->{children}->[0];
        my string $parent_include_semicolon = $parent_include->{children}->[2];
        $rperl_source_group->{PMC} .= $parent_include_use_keyword . q{ } . $parent_include_parent_name . $parent_include_semicolon . "\n";
...
#        RPerl::diag( 'in Class::Generator->ast_to_rperl__generate(), have $parent_include_elements = ' . "\n" . RPerl::Parser::rperl_ast__dump($parent_include_elements) . "\n" );
        foreach my object $parent_include_element (@{$parent_include_elements->{children}}) {
#            RPerl::diag( 'in Class::Generator->ast_to_rperl__generate(), have $parent_include_element = ' . "\n" . RPerl::Parser::rperl_ast__dump($parent_include_element) . "\n" );
            my string $parent_include_element_name = $parent_include_element->{attr};
            if ($parent_include_element_names ne q{}) { $parent_include_element_names .= q{ }; }
            $parent_include_element_names .= $parent_include_element_name;
        }
        my string $parent_include_right_parenthesis = $parent_include->{children}->[4];
        my string $parent_include_semicolon = $parent_include->{children}->[5];
        $rperl_source_group->{PMC} .= $parent_include_use_keyword . q{ } . $parent_include_parent_name . q{ } . $parent_include_qw . $parent_include_element_names . $parent_include_right_parenthesis . $parent_include_semicolon . "\n";
    }

    foreach my object $critic (@{$critic_star->{children}}) {
        my string__hash_ref $rperl_source_subgroup = $critic->ast_to_rperl__generate($modes);
        RPerl::Generator::source_group_append($rperl_source_group, $rperl_source_subgroup);
    }

    foreach my object $include (@{$include_star->{children}}) {
        my string__hash_ref $rperl_source_subgroup = $include->ast_to_rperl__generate($modes);
        RPerl::Generator::source_group_append($rperl_source_group, $rperl_source_subgroup);
    }

    foreach my object $constant (@{$constant_star->{children}}) {
        my string__hash_ref $rperl_source_subgroup = $constant->ast_to_rperl__generate($modes);
        RPerl::Generator::source_group_append($rperl_source_group, $rperl_source_subgroup);
    }
    
    if (ref $properties eq 'Properties_63') {
        # ...
    }
    else {  # Properties_64
    ...
    foreach my object $method_or_subroutine (@{$method_or_subroutine_star->{children}}) {
        my string__hash_ref $rperl_source_subgroup = $method_or_subroutine->ast_to_rperl__generate($modes);
        RPerl::Generator::source_group_append($rperl_source_group, $rperl_source_subgroup);
    }
